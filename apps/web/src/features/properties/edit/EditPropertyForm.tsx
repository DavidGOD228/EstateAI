...
import { FeatureTagsEditor } from '../create/FeatureTagsEditor';
import {
  MAX_ADDRESS_LENGTH,
  MAX_CITY_LENGTH,
  MAX_COUNTRY_LENGTH,
...
import type { DraftValues } from '../create/types';
import { useEditProperty } from './useEditProperty';

export function EditPropertyForm({ property }: { property: PropertyDto }) {
  const navigate = useNavigate();
  const { values, errors, status, errorMessage, setField, addFeature, removeFeature, save } =
  ...
  return (
    <div className="flex flex-col gap-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Update the details below and save your changes.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Button type="button" variant="primary" loading={pending} disabled={pending} onClick={save}>
          Save changes
        </Button>
        <Button
          type="button"
